...
#if defined(SCRIPTCORE_BACKEND_V8) || defined(SCRIPTCORE_BACKEND_JAVASCRIPTCORE) || defined(SCRIPTCORE_BACKEND_LUA)
...
TEST_F(ByteBufferTest, IsInstance) {
  ...
  auto ret = engine->eval(TS().js("buffer instanceof ArrayBuffer")
                              .lua("return ScriptCore.isInstanceOf(buffer, ByteBuffer)")
                              .select());
...
TEST_F(ByteBufferTest, wasm_interop_newSharedByteBuffer) {
  std::weak_ptr<void> weak;
  {
    ...
    EXPECT_TRUE(engine->eval("buffer instanceof ScriptCore.SharedByteBuffer").asBoolean().value());
...
buffer = new ScriptCore.SharedByteBuffer(16);
if (buffer.byteOffset == 0 || buffer.byteLength != 16 || !(buffer.buffer instanceof ArrayBuffer)) {
  throw Error("new ScriptCore.SharedByteBuffer(16); test failed");
}
buffer;
...
TEST_F(ByteBufferTest, IsSharedByteBuffer) {
  ...
  auto shared1 = engine->eval("new ScriptCore.SharedByteBuffer(0)").asByteBuffer();
...
#if defined(SCRIPTCORE_LANG_JAVASCRIPT) && !defined(SCRIPTCORE_BACKEND_WEBASSEMBLY)
