...
class TestClass : public ScriptClass {
 public:
  std::string src = "https://github.com/marcprux";
        ...
        (function() {
          return function (ins) {
              return ins instanceof InstanceOfTest;
                               ...
                               .lua("return function (ins) return ScriptCore.isInstanceOf(ins, "
                                    "InstanceOfTest) end")
                               .select())
                    .asFunction();
...
#ifdef SCRIPTCORE_NO_EXCEPTION_ON_BIND_FUNCTION
...
TEST_F(NativeTest, WasmScriptClassDestoryTest) {
  ...
  engine->eval(R"(
    ...
    ScriptCore.destroyScriptClass(ins);
  )");
  EXPECT_TRUE(WasmScriptClassDestroyScriptClass::destroyed_);
}
